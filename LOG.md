...
- 2026-05-19: 통합 엔트리 포인트(`gocator`) 구축. 원인: CLI/UI 실행 파일 분산으로 인한 관리 복잡도. 결과: 인자 유무에 따른 자동 모드 전환 및 단일 바이너리 관리.
- 2026-05-19: `runDebug` 진단 툴 구현. 원인: 센서 연결 및 이미지 미출력 원인 파악 지연. 결과: 단계별(접속-탐색-상태-프로토콜-출력-수신) 상태 점검 및 원인 분석 자동화.
- 2026-05-19: 이미지 획득 파이프라인 검증 및 개선. 원인: Surface(2D) 지원 누락 및 16-bit 데이터 가독성 저하. 결과: `UNIFORM_SURFACE` 지원, 16-bit Normalization 적용, 실측 데이터(이미지/프로파일) 무결성 확인.
